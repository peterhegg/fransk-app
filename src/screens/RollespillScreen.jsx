    ...
    if (loadError) return (
      <div style={{ minHeight: "100dvh", background: "var(--bg)", display: "flex", flexDirection: "column" }}>
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16, padding: 32 }}>
          ...
          <PrimaryButton onClick={() => startScenario(scenario)}>Prøv igjen</PrimaryButton>
          <GhostButton onClick={() => setPhase("select")} style={{ padding: "10px 20px", fontSize: 13 }}>← Velg scenario</GhostButton>
        </div>
        {nav}
      </div>
    );
    return <LoadingState icon={scenario?.icon} label="Pierre forbereder seg…" bottomNav={nav} />;
  }
  ...
  if (phase === "result") {
    ...
    return (
      <GameResult
        icon={dice}
        title={`${score} / 6`}
        subtitle={result?.comment}
        primary={{ label: "Spill igjen", onClick: () => startScenario(scenario) }}
        secondary={{ label: "Velg scenario", onClick: () => setPhase("select") }}
        bottomNav={nav}
      >
        {result?.corrections?.filter(Boolean).map((c, i) => (
          <div key={i} style={{ width: "100%", maxWidth: 320, fontSize: 12, color: "var(--text-subtle)", background: "var(--color-error-bg)", border: "1px solid var(--color-error-border)", borderRadius: 10, padding: "10px 14px", fontFamily: "var(--font-body)", lineHeight: 1.6 }}>⚠ {c}</div>
        ))}
      </GameResult>
    );
  }
  ...
  return (
    <div style={{ minHeight: "100dvh", background: "var(--bg)", display: "flex", flexDirection: "column" }}>
      ...
      <GameHeader
        ...
        icon={scenario?.icon}
        right={<>{noToggle}<CountPill>{turn}/{MAX_TURNS}</CountPill></>}
      />

      <GameProgress total={MAX_TURNS} current={turn} />

      {/* Chat */}
      <div style={{ flex: 1, overflowY: "auto", padding: "0 20px", display: "flex", flexDirection: "column", gap: 10, paddingBottom: 300 }}>
        {messages.map((m, i) => (
      ...
      {options.length > 0 && !busy && (
        <Dock>
          <div style={{ display: "flex", flexDirection: "column", gap: 7, width: "100%" }}>
            {options.map((opt, i) => (
              <button key={i} onClick={() => pickOption(opt)} className="press"
